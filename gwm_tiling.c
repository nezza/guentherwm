...
void gwm_tiling_init(gwm_workspace *spc) {
	...
	spc->organizer->add_window = gwm_tiling_org_add_window;
	spc->organizer->remove_window = gwm_tiling_org_remove_window;
	spc->organizer->free_organizer = gwm_tiling_org_free;
	spc->organizer->move_left = gwm_tiling_org_move_window_left;
	spc->organizer->move_right = gwm_tiling_org_move_window_right;
	spc->organizer->focus = gwm_tiling_org_focus;
	spc->organizer->focus_window = gwm_tiling_org_focus_window;
	spc->organizer->select_left = gwm_tiling_org_select_left;
	spc->organizer->select_right = gwm_tiling_org_select_right;
...
// Functions to implement an organizer
...
void gwm_tiling_org_focus(gwm_workspace *spc) {
	gwm_tiling_data *data = spc->organizer->data;
	gwm_tiling_column_focus(data->active_column);
}
...
unsigned gwm_tiling_column_count_windows(gwm_tiling_column *column) {
    gwm_tiling_window *win;
    unsigned ret = 0;
    for(win = column->wins; win; win = win->next, ret++);
    return ret;
}
...
void gwm_tiling_move_resize(gwm_window *win) {
	...
	gwm_window_resize(win, cw - (2*CONFIG_BORDER_WIDTH),
			win->spc->gwm->sh - (2*CONFIG_BORDER_WIDTH));
}
